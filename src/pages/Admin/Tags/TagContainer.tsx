...
const Tags: React.FC = () => {
  ...
  const { isLoading, data } = useQuery({
    ...
    queryFn: async () => {
      try {
        const data = await getAdminTagsRequest(params);
  ...
  const createTag: UseMutationResult = useMutation({
    mutationFn: async (data) => await creatTagAdmin(data),
    onSuccess: () => {
      setParams({
        ...params,
        keySearch: tag.name,
  ...
  const updateTag: UseMutationResult = useMutation({
    mutationFn: async (data) => await updateTagAdmin(data),
    onSuccess: () => {
      onCloseAlert();
      onClose();
      toast({
  ...
  const deleteTag: UseMutationResult = useMutation({
    mutationFn: async (data) => await deleteTagAdmin(data),
    onSuccess: () => {
      onCloseAlert();
      onClose();
      queryClient.invalidateQueries({ queryKey: ["Tags"] });
  ...
  return (
    <>
      <Box>
        <Skeleton isLoaded={!isLoading}>
          ...
          <Box>
            {data && (
              <CommonTable
                data={data.data.NumberOfPostsPerTag}
                columns={columns}
              />
            )}
            <Pagination
      ...
      <Modal isOpen={isOpenModal} onClose={onClose} size={"xl"}>
        <ModalOverlay />
        <ModalContent>
          <ModalHeader>{isEdit ? "Edit Tag #" : "Create Tag #"}</ModalHeader>
          <ModalCloseButton />
          <ModalBody>
            <Grid
